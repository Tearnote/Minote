...
//			play.tick(updateActions);
		...
		engine.enqueue("block"_id, std::array{
			gfx::Engine::Instance{
				.transform = make_translate({0.0f, 0.0f, 0.0f}) * make_scale({12.0f, 1.0f, 12.0f}) * glm::inverse(rotateTransform),
				.tint = {0.9f, 0.9f, 1.0f, 1.0f},
			},
			gfx::Engine::Instance{
				.transform = make_translate({-4.0f, 2.0f, -4.0f}) * rotateTransform,
			...
			gfx::Engine::Instance{
				.transform = make_translate({0.0f, 2.5f, 0.0f}) * make_scale({1.5f, 1.5f, 1.5f}) * rotateTransformAnim * rotateTransform,
		...
		for (auto i = 0.0f; i <= 1.0f; i += 0.125f)
			engine.enqueue("sphere"_id, std::array{gfx::Engine::Instance{
				.transform = make_translate({(i - 0.5f) * 2.0f * 8.0f, 2.0f, 8.0f}),
				.roughness = i,
				.metalness = 0.9f,
			}});
		for (auto i = 0.0f; i <= 1.0f; i += 0.125f)
