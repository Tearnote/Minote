...
auto InstanceList::cull(ModelBuffer& _models, ObjectBuffer& _objects,
	...
	rg->attach_image("stub", vuk::ImageAttachment{
		.extent = vuk::Dimension3D::absolute(1, 1),
		.format = vuk::Format::eR8Unorm,
		.sample_count = vuk::Samples::e1,
		.level_count = 1,
		.layer_count = 1,
	}); //FIXME remove the binding stub, pending vuk fixing unused bindings
	if (_hiz)
	...
	auto resources = std::vector<vuk::Resource>{
		...
		"stub"_image >> vuk::eComputeSampled,
	};
	if (_hiz)
	...
	rg->add_pass(vuk::Pass{
		...
		.execute = [this, _view, _projection, useHiz=_hiz.has_value()](auto& cmd) {
...
TriangleList::TriangleList(vuk::Allocator& _allocator, ModelBuffer& _models, InstanceList& _instances) {
	...
	rg->add_pass(vuk::Pass{
		...
		.execute = [instanceCount=_instances.instanceCount](auto& cmd) {
