...
void Antialiasing::quadResolve(vuk::RenderGraph& _rg, Texture2D _target, Texture2D _velocity,
	...
	Texture2D _targetPrev, Texture2D _quadbufPrev, Texture2D _outputsPrev, Buffer<World> _world) {
	...
	_rg.add_pass({
		...
		.execute = [_quadbuf, _jitterMap, _outputs, _targetPrev, _quadbufPrev, _outputsPrev,
			...
			cmd.bind_uniform_buffer(0, 0, _world)
			   ...
			   .bind_sampled_image(0, 7, _velocity, LinearClamp)
			   .bind_storage_image(0, 8, _target)
