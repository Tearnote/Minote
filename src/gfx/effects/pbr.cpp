...
void PBR::apply(Frame& _frame, QuadBuffer& _quadbuf, Worklist _worklist,
	InstanceList _instances, Texture2D _shadowOut, Cubemap _ibl,
	...
	_frame.rg.add_pass({
		...
		.resources = {
			...
			_shadowOut.resource(vuk::eComputeSampled),
		...
		.execute = [_quadbuf, _worklist, &_frame, _instances, _shadowOut, _ibl,
			...
			cmd.bind_uniform_buffer(0, 0, _frame.world)
			   ...
			   .bind_sampled_image(0, 6, _shadowOut, NearestClamp)
