...
class Window {
public:
	using KeyHandlerCallback = std::function<void(int key, int action)>;
	...
	// Returns true if a signal has been sent for the window to close.
	// When this happens, destroy this object ASAP.
	auto isOpen() -> bool;

	// Pass a function to run every time a key is pressed inside the window
	auto registerKeyHandler(KeyHandlerCallback) -> void;
	auto unregisterKeyHandler() -> void;
