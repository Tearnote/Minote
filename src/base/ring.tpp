...
ring<T, Capacity>::ring(size_type const num) {
	for (size_type i = 0; i < num; ++i)
		emplace_back();
}

template<typename T, std::size_t Capacity>
ring<T, Capacity>::ring(size_type const num, value_type const& val) {
	for (size_type i = 0; i < num; ++i)
		emplace_back(val);
}
...
ring<T, Capacity>::ring(InputIt first, InputIt last) {
	std::for_each(first, last, [this](auto const& val) { emplace_back(val); });
}

template<typename T, std::size_t Capacity>
ring<T, Capacity>::ring(std::initializer_list<value_type> il) {
	std::for_each(il.begin(), il.end(), [this](auto const& val) { emplace_back(val); });
}

template<typename T, std::size_t Capacity>
void ring<T, Capacity>::swap(ring& other) {
...
void ring<T, Capacity>::push_back(const_reference value) {
	if (length == capacity())
...
template<typename T, size_t Capacity>
void ring<T, Capacity>::push_back(value_type&& value) {
	if (length == capacity())
		throw std::out_of_range{"ring is full"};
...
template<typename T, std::size_t Capacity>
void ring<T, Capacity>::push_front(const_reference value) {
	if (length == capacity())
		throw std::out_of_range{"ring is full"};
...
template<typename T, std::size_t Capacity>
void ring<T, Capacity>::push_front(value_type&& value) {
	if (length == capacity())
		throw std::out_of_range{"ring is full"};
...
auto ring<T, Capacity>::emplace_back(Args&&... args) -> reference {
	if (length == capacity())
		throw std::out_of_range{"ring is full"};
...
template<typename T, size_t Capacity>
template<typename... Args>
auto ring<T, Capacity>::emplace_front(Args&&... args) -> reference {
	if (length == capacity())
		throw std::out_of_range{"ring is full"};
...
template<typename T, std::size_t Capacity>
auto ring<T, Capacity>::operator=(std::initializer_list<value_type> il) -> ring& {
	clear();
	std::for_each(il.begin(), il.end(), [this](auto const& val) { emplace_back(val); });
}
