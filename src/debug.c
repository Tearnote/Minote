...
static const char* ProgramNuklearVertName = u8"nuklear.vert";
static const GLchar* ProgramNuklearVertSrc = (GLchar[]){
#include "nuklear.vert"
	'\0'};
static const char* ProgramNuklearFragName = u8"nuklear.frag";
...
static struct nk_draw_null_texture nullTexture = {0};
...
static atomic point2i cursorPos;
static atomic bool leftClick;
static atomic bool rightClick;
...
void debugInit(void)
{
	...
	nuklear->atlas = programSampler(nuklear, u8"atlas", GL_TEXTURE0);
	nuklear->projection = programUniform(nuklear, u8"projection");
...
#define null 0
	...
	nk_draw_foreach(command, &nkContext, &commandList) {
		...
		textureUse(command->texture.ptr, nuklear->atlas);
